...
public class ServoSettings implements Serializable {
	...
	public static ServoSettings loadCorrectionsFrom(File file, String robotarFolder) {
		...
		try {
			...
			String pathStr = System.getProperty("user.home") + File.separator 
					+ robotarFolder + File.separator
					+ file.getPath();
			//Path path = Paths.get(pathStr);
			File otherFile = new File(pathStr);
			LOG.info("loading corrections from file: {}", otherFile.getAbsolutePath());
			return loader.load(new FileInputStream(otherFile));
		} catch (IOException e) {
			LOG.error("loadfrom.ioexception", e);
		} catch (ParserConfigurationException e) {
			LOG.error("loadfrom.parseconfigurationexception", e);
